...
/**
 * Owns maintenance-record server state for one vehicle: the current page of records plus the
 * query (sort/filter/page/size) that produced it, and the summary aggregates computed over the
 * whole dataset. `load()` (and every query/page/size change) is fire-and-forget and safe to call
...
export class MaintenanceStore {
  ...
  private readonly _size = signal(DEFAULT_MAINTENANCE_QUERY.size);
  ...
  // `MaintenanceList` don't lose their own local UI state (e.g. an open filter modal) on every
  // query change.
    ...
    this.load$
      .pipe(
        ...
        switchMap(({ vehicleId, query }) =>
          this.maintenanceApi.getPage(vehicleId, query).pipe(
            catchError(() => {
  ...
  setQuery(partial: Partial<Omit<MaintenanceQuery, 'page' | 'size'>>): void {
